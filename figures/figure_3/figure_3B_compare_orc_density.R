...
chip_total.df = read.csv(paste("/data/data2/jab112/2014_mnase_manuscript/figures/figure_3/figure_3_datasets/",
			       "oridb_acs_feature_file_curated_798_sites_orc_chip_seq_left_win_400bp_right_win_400bp.csv", 
			      sep = ""
			     )
		       )
...
chip_total.df[,"chip"] = (chip_total.df$orc_chip_seq_dm265 + chip_total.df$orc_chip_seq_dm287) / 2
chip_total.df[,"input"] = (chip_total.df$input_chip_seq_dm271_orc + chip_total.df$input_chip_seq_dm272_orc) / 2
...
# Set up the plot
plot(0, 0, type = "n", 
     ylab = expression(paste(log[2], " ratio ORC ChIP-seq to input")), xlab = "",
...
for(i in 1:3){
	...
	chip_subset.v = chip_subset.df$log2_diff[which(chip_subset.df$chip >= 130 & chip_subset.df$input >= 130)]
...
text(x = c(1.5, 2, 2.5), y = c(y_left, y_outer, y_right) + 0.1, labels = "*", cex = 1.25)
